 ...
 * 
 * @brief This C++ source file implements the trieber_stack, contains the tstack class 
 *        and members push and pop
 * @date 4 Dec 2023
********************************************************************/
...
void tstack::push(int val){
    ...
    do {
        ...
        n->down.store(old_top.ptr, RELAXED); // Set the new node's next pointer to the current top
    ...
    } while(!cas(top, old_top, new_top, ACQ_REL)); // This is the linearization point of push
}
...
int tstack::pop(void){
    ...
    do {
        ...
        node* next_node = old_top.ptr->down.load(RELAXED); // Get the next node
        v = old_top.ptr->val.load(RELAXED); // Read the value from the current top node
        new_top.ptr = next_node; // Update the new top to the next node
        new_top.count = old_top.count + 1; // Increment the counter to address the ABA problem

        // Attempt to swap the old top with the new top.
        // If another thread has modified the top, the CAS will fail and retry.
    } while(!cas(top, old_top, new_top, ACQ_REL)); // This is the linearization point of pop

    // Memory reclamation should be performed here.
    delete old_top.ptr;// Delete the old node after ensuring no other threads are accessing it.
